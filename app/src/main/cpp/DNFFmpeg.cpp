...
void DNFFmpeg::prepareFFmpeg() {
    ...
    av_err2str(ret);
    LOGE("%s DNFFmpeg ret:%d  %s", url, ret, av_err2str(ret));
    LOGE(" DNFFmpeg ---------  创建%s",url);
    if (ret != 0) {
        if (javaCallHelper) {
    ...
    LOGE("DNFFmpeg ---------  duration is: %lld, nb_stream is: %d",formatContext->duration, formatContext->nb_streams);
